...
Nushell 与 bash/zsh/fish 不同：在命令行第一个位置（命令位置）按 Tab 时，**只会补全命令名**（内置命令 + PATH 中的可执行文件），不会补全当前目录下的普通文件或目录。

```nu
# ❌ 不会补全文件/目录
co<Tab>       # 只显示匹配的命令名，不显示 config/ 等目录

# ✅ 需要加 ./ 前缀才能补全文件/目录
./co<Tab>     # 会补全 config/ 等当前目录下的文件/目录
./<Tab>       # 列出当前目录下所有文件/目录
```

> 💡 **适用场景**：当你记不清文件/目录全名，想用 Tab 补全时，记得先输入 `./`。
